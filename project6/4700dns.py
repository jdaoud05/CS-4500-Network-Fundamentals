...
# put name/type into consistent key
...
# loop through records and call put() for each
    def put_many(self, rrs, now=None):
        now = now or time.time()
        for rr in rrs:
            self.put(rr, now)

# Look up cached DNS records by name and type
# Filter out expired entrys
...
def send_and_recv(sock, ip, qname, qtype, timeout=TIMEOUT):
    try:
        ...
        q.header.rd = 0   # don't ask upstreams to recurse
...
# loop up to 40 iterations to avoid infinite loops
        for _ in range(40):
            log("  [resolve] %s %s @ %s (zone=%s)" % (
                qname, QTYPE[qtype], ns_ip, zone))
            resp = query_server(sock, ns_ip, qname, qtype)

            # if no usable response, return  most recent response
            if resp is None:
...
# If response contains answers, return clean DNS response
            if resp.rr:
...
# Decide authoritative vs recursive response
                ...
                for mr in direct:
                    ...
                    if self._is_auth(mx_name):
                        for a in zlookup(self.zone_idx, mx_name, QTYPE.A):
                            resp.add_ar(a)
            # NS queries have their records in ANSWER already, so skip authority
            if qtype != QTYPE.NS:
                self._add_ns_authority(resp)
            return resp
...
# Wraps resolver output back to client
